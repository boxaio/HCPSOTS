...
template<>
inline complex zero<complex>() 
{
    return complex(0., 0.);
}

inline scalar distance(const complex& a, const complex& b) 
{
    return std::abs(a-b);
}

template<int N>
inline scalar distance(const Eigen::Vector<scalar, N>& a, const Eigen::Vector<scalar, N>& b) 
{
    return (a-b).norm();
}
