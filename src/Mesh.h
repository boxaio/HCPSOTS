...
struct Mesh 
{
    ...
    scalar distanceToTriangle(const vec& p, const Face& face) const
    {
        ...
        const vec& b = V[1];
        const vec& c = V[2];
        static const auto cross = [] (const vec& a, const vec& b) { return a.cross(b);};
        static const auto dot = [] (const vec& a, const vec& b) { return a.dot(b);};
        static const auto dot2 = [] (const vec& a) { return a.dot(a);};
        static const auto sign = [] (const scalar& a) { return (a < 0) ? -1 : 1;};
    ...
    */
    ...
    bool rayTriangleIntersection(const vec& dir, const vec& v0, const vec& v1, const vec& v2) const 
    {  // compute the plane's normal
        vec v0v1 = v1 - v0;
        vec v0v2 = v2 - v0;
        // no need to normalize
