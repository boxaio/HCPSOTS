...
# https://github.com/scipython/scipython-maths/blob/master/poisson_disc_sampled_noise/poisson.py
...
class UniformNoise():
    ...
    def reset(self):
        pass
    def sample(self):
        return np.array([np.random.uniform(0, self.width, size=self.n),
                         np.random.uniform(0, self.height, size=self.n)]).T